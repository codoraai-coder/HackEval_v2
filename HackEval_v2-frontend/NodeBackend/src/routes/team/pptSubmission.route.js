...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    if (!extension) {
      ...
      else if (file.mimetype.includes("presentation")) extension = ".pptx";
      else extension = ".ppt";
    }
    cb(null, "ppt-" + uniqueSuffix + extension.toLowerCase());
  },
});
const fileFilter = (req, file, cb) => {
  const allowedMimeTypes = [
  ...
  if (
    ...
    cb(
      new Error("Only PowerPoint (.ppt, .pptx) and PDF files are allowed"),
      false,
    );
};
const upload = multer({
...
// Webhook route for external evaluator
router.post("/webhook/evaluation-result", receiveEvaluationWebhook);

// Start stuck-job resend sweep
initResendSweep();
